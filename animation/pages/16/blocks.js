...
Blockly.Blocks['pour_chaque_image'] = {
    init: function () {
        this.appendDummyInput()
            .appendField("Pour les ")
            .appendField(new Blockly.FieldNumber(10, 1, 30), "frames")
            .appendField(" images Faire");
...
Blockly.JavaScript['pour_chaque_image'] = function (block) {
    var number_frames = block.getFieldValue('frames');
    var statements_image = Blockly.JavaScript.statementToCode(block, 'image').replace(/(\r\n|\n|\r)/gm, "");

    var code = 'for(let i = 0; i < ' + number_frames + '; i++) {currentFrame = i;' + statements_image + '};';
    return code;
};
